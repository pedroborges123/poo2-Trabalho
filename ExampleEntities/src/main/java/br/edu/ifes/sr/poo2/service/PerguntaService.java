...
public class PerguntaService implements CRUDService<Long, Pergunta> {
	...
	public Pergunta cadastrar(Pergunta pergunta) throws Exception {

		if (!isPergunta(pergunta.getValor())) {
			save(pergunta);
			return pergunta;
		} else {
	...
	public List<Pergunta> getRandom(int quantPerguntas, Nivel nivel)
			throws Exception {
